...
**Roles.** Built-ins: Worker (execute in isolated workspace), Integrator (merge queue, conflict resolution), Coordinator (spawn/assign/broadcast), Monitor (read-only health watch), Generic (minimal base). Teams define custom roles via `extends` in `roles/<name>.yaml`. Tool registration is capability-gated: `isToolAllowedForRole()` checks before each MCP tool is registered.
...
Each streamed agent gets an isolated git worktree via `WorkspaceManager`, backed by `git-cascade`'s `MultiAgentRepoTracker`. Commits made through `commitChanges()` get a stable `Change-Id: c-xxxxxxxx` trailer that survives rebases; legacy callers using raw `git commit` bypass this tracking. The old `workspace/merge-queue/` and `workspace/strategies/` modules are `@deprecated` — superseded by `git-cascade`'s built-in merge queue and `workspace/landing/`, respectively.
...
Set by boot / system config: `MACRO_BASE_DIR` (default `~/.macro-agent`), `MACRO_WORKSPACE_POOL_SIZE` (default `10`), `MACRO_MERGE_QUEUE_DB` (default `:memory:`), `MACRO_TEAMS` (comma-separated auto-start teams), `MACRO_AGENT_HOME` (alt base dir for CLI clear).
